...
3. Key 只保存在当前页面的 JavaScript 内存中，并直接发送到 `https://api.deepseek.com`。
4. 刷新或关闭页面后 Key 自动清除，下次使用需要重新输入。
...
- Key 不得写入源码、GitHub Actions、构建产物、Web Storage、IndexedDB 或聊天。
- 页面使用 CSP 限制脚本和网络目标，并在 iframe 嵌入模式下停止加载工作区和 Key 输入。
- 独立后端访问码也只保存在页面内存中，不再写入 `sessionStorage`。
- 本地备份可使用密码加密；导入前会校验完整数据结构，拒绝属性注入、外部图片地址和异常大小。
