...
SaaS 模式下 Agent 路由还会逐请求验证企业成员和编辑权限；`draft` 必须带幂等键，事务预留一篇额度，失败释放、成功提交。模型 Token 和估算成本按企业/用户记录。发送给 DeepSeek 的 `user_id` 是经 HMAC 匿名化的企业标识，不包含邮箱或企业名称。所有 Agent POST 请求仍统一执行请求体、媒体类型、来源/全局限流、CORS、超时和最小错误响应。
